...
% Subject exclusion
nomiFile([2,3,13,16,28,40,47,50,51,55,58,59,66,68,70]) = [];
...
wavelet_stride_duration_CO = []; % Stride time wavelet (control)
wavelet_stride_duration_FL = []; % Stride time wavelet (fallers)
...
for k = 1:length(nomiFile)
   ...
   [SMA, energy] = SMA_energy_func(window,aV_filt, aML_filt, aAP_filt, Fs, limit);
    ...
    if record_name(end-4) == 'C'
        ...
        wavelet_stride_duration_CO = [wavelet_stride_duration_CO, wavelet_stride_duration];
        ...
        wavelet_stride_duration_FL = [wavelet_stride_duration_FL, wavelet_stride_duration];
