clc, clear, close all
% here the struct creation was not possible due to the high volume of data and we were forced to use another method
...
% Subject exclusion due to: 60h or less of activity (because 72h were
% requested --> lower values measured and can affect the data), then the
% subjects with the highest and lowest activity percentage (due to a
% problem in variability --> tradeoff between acceptable level of
% variability and lowest possible number of subjects eliminated
% fixing further (removing other subjects) results in a chasing of paper results and not persuing our methodology

nomiFile([1,2,13,14,25,26,29,47,51,58,63,66,68]) = [];  % 13 subjects

% preaalocating the arrays
...
threshold_en = 0.05;
% SMA thresholds from other articles 
threshold_SMA_min = 0.135;
threshold_SMA_max = 0.8;

% High-pass filter creation for data preprocessing (with the Moe Nilssen)
% similar to the Lab Analysis
Wn = 0.5 / (Fs / 2);
order = 4;
[B_hp, A_hp] = butter(order, Wn, 'high');

% Low-pass filter for step detection --> increases the smoothness of the
% signal and easier detection of peaks 15Hz
Wn = 15 / (Fs / 2);
[B_lp, A_lp] = butter(order, Wn);

for k = 2:2% length(nomiFile) %for every subject we have performed...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Walking detection (OR logic) from the article formula
...
output_func(walking_percentage_CO, walking_percentage_FL, total_steps_CO, total_steps_FL, stride_time_CO, stride_time_FL, valid_bouts_CO, valid_bouts_FL, avg_range_CO, avg_range_FL, total_minutes_CO, total_minutes_FL)
