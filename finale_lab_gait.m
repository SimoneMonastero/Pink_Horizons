...
for i=1:lengthC
   ...
   [aAP, aML, aV] = algo_Moe_Nilssen(accData(:,3), accData(:,2), accData(:,1) ,'tiltAndNoG');
    ...
    [paAP,f]=pwelch(aAP, [], [], [], Fs);    % Pwelch vuole anche la sampling frequency, se no di default è 1Hz
    paAP=paAP(f>0.5 & f<3);
    f=f(f>0.5 & f<3);
    [ampl, ind]=max(paAP); % amplitude psd che è passi/secondo
...
for i=1:lengthF
   ...
   [aAP, aML, aV] = algo_Moe_Nilssen(accData(:,3), accData(:,2), accData(:,1) ,'tiltAndNoG');
    ...
    step_length=2*sqrt(2*l_fall*h - h^2); % metri/passo
